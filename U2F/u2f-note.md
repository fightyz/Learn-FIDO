...
2. u2f设备也可以存储这个"wrapped"信息在se之外，u2f设备之上。这种情况下，key handle就可以是一个索引值（在这种方案下，U2F设备依然能确定RP发送过来的Key Handle就是该U2F设备生成的，因为可以根据Key Handle这个索引，找到"wrapped"的信息（包括私钥和appid的hash））
